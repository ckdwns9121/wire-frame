...
const OAuth =({match,location})=>{
    ...
    useEffect(()=>{
        ...
        if (isMobile()) {
            if (type === 'login') {
                window.location.href= `http://devm.ajoonamu.com/oauth/login?email=${email}&access_token=${access_token}&register_type=${register_type}`;
            } else if (type === 'register') {
                window.location.href= `http://devm.ajoonamu.com/oauth/register?name=${name}&email=${email}$register_kind=1&register_type=${register_type}`;
            }
        } 
        //pc이면
        else {
